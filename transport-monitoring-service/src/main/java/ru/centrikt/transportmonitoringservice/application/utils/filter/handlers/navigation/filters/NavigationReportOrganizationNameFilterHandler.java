...
@Component
public class NavigationReportOrganizationNameFilterHandler implements CustomFilterHandler<NavigationReport> {
    @Override
    public boolean supports(String fieldName) {
        return "organizationName".equals(fieldName);
    }

    @Override
    public void applyFilter(Root<NavigationReport> root, CriteriaBuilder cb, List<Predicate> predicates, String rawValue) {
        predicates.add(cb.equal(root.get("sensor").get("organizationName"), rawValue));
    }
}

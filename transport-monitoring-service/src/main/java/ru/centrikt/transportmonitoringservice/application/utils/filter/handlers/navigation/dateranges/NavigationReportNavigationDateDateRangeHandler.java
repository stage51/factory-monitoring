package ru.centrikt.transportmonitoringservice.application.utils.filter.handlers.navigation.dateranges;
...
@Slf4j
@Component
public class NavigationReportNavigationDateDateRangeHandler implements CustomDateRangeHandler<NavigationReport> {

    private static final ZoneId USER_TIMEZONE = ZoneId.of(DateTimeConfig.getDefaultValue());
    
    @Override
    public boolean supports(String fieldName) {
        return "navigationDate".equals(fieldName);
    }

    @Override
    public void applyDateRange(Root<NavigationReport> root, CriteriaBuilder cb, List<Predicate> predicates, String rawValue) {
        ...
        try {
            if (!"null".equals(dates[0])) {
                ...
                predicates.add(cb.greaterThanOrEqualTo(root.get("navigationData").get("navigationDate"),
                        start.toLocalDateTime()));
            }
            if (!"null".equals(dates[1])) {
                ZonedDateTime end = ZonedDateTime.parse(dates[1], DateRange.FORMATTER)
                        .withZoneSameInstant(USER_TIMEZONE);
                predicates.add(cb.lessThanOrEqualTo(root.get("navigationDate").get("navigationDate"),
                        end.toLocalDateTime()));
            }
        } catch (DateTimeParseException ex) {
            log.error("Date parse error for '{}': {}", "navigationDate", ex.getMessage());
        }
    }
}

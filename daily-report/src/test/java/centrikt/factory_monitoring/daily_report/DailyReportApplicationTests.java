...
import centrikt.factory_monitoring.daily_report.utils.jwt.JwtTokenUtil;
	...
	class PositionProductIntegrationTests {
		...
		void testCRUDAndProductRemovalWithPosition() {
			...
			ResponseEntity<PositionResponse> getResponse = restTemplate.exchange(
					"/api/v1/daily-report/positions/{id}",
					HttpMethod.GET,
			...
			ResponseEntity<ProductResponse> productResponse = restTemplate.exchange(
					"/api/v1/daily-report/products/{id}",
					HttpMethod.GET,
					new HttpEntity<>(headers),
		...
		void testBatchCreation() {
			...
			for (int i = 0; i < 5; i++) {
				...
				req.setSensorNumber("12_5" + i);
				ZonedDateTime now = ZonedDateTime.now();
				req.setStartDate(now);
				req.setEndDate(now.plusHours(1));

				req.setVbsStart(new BigDecimal("1000.00"));
				req.setVbsEnd(new BigDecimal("1500.00"));
				req.setAStart(new BigDecimal("500.00"));
				req.setAEnd(new BigDecimal("600.00"));
				req.setPercentAlc(new BigDecimal("96.5"));
				req.setBottleCountStart(new BigDecimal("1000"));
				req.setBottleCountEnd(new BigDecimal("1200"));
				req.setTemperature(new BigDecimal("25.5"));
			...
			ResponseEntity<List<PositionResponse>> response = restTemplate.exchange(
					"/api/v1/daily-report/positions/list",
					HttpMethod.POST,
					entity,
		...
		void testUploadFile() throws Exception {
			...
			ResponseEntity<String> response = restTemplate.exchange(
					baseUrl + "/api/v1/daily-report/positions/upload",
					HttpMethod.POST,
					entity,
					String.class
			);
	...
	class PositionProductPaginationTests {
		...
		void testCreatePositionAndProductPagination() {
			...
			ResponseEntity<TestPage<PositionResponse>> positionPageResponse = restTemplate.exchange(
					"/api/v1/daily-report/positions/fetch/{taxpayerNumber}",
					HttpMethod.POST,
					requestEntity,
			...
			ResponseEntity<TestPage<ProductResponse>> productPageResponse = restTemplate.exchange(
					"/api/v1/daily-report/products/fetch",
					HttpMethod.POST,
